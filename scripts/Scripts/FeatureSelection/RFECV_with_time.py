...
df_time = pd.read_csv(r'C:\Users\Lorenzo\Desktop\Progetto DSL\features//training_set_time_features_blackmanharris.csv', header=0)
df_ZRC = pd.read_csv(r'C:\Users\Lorenzo\Desktop\Progetto DSL\features//ZCR_on_training_set.csv', header=0)
df_ZRC.drop(columns = 'Unnamed: 0', inplace = True)
df_time.drop(columns = 'Unnamed: 0', inplace = True)

list_df_to_concat.append(df_MelSpec)
list_df_to_concat.append(df_time)
list_df_to_concat.append(df_ZRC)
#%%
...
df_eval_time = pd.read_csv(r'C:\Users\Lorenzo\Desktop\Progetto DSL\features//evaluation_set_time_features_blackmanharris.csv', header=0)
df_eval_time.drop(columns = 'Unnamed: 0', inplace = True)

df_eval_ZCR = pd.read_csv(r'C:\Users\Lorenzo\Desktop\Progetto DSL\features//ZCR_on_evaluation_set.csv', header=0)
df_eval_ZCR.drop(columns = 'Unnamed: 0', inplace = True)

df_eval = pd.concat([df_eval, df_eval_time], axis = 1)
df_eval = pd.concat([df_eval, df_eval_ZCR], axis = 1)
...
min_features = 200
rfecv = RFECV(estimator = clf, step = 10, cv=CV, scoring = 'accuracy', min_features_to_select = min_features, n_jobs = -1)
