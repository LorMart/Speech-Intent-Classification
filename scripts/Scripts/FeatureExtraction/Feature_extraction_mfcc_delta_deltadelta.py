...
def extract_spec_feat_from_signal(signal, sample_rate):
    ...
    mfcc = librosa.feature.mfcc(S=S_db, sr=sample_rate, n_mfcc=13)
    mfcc_delta = librosa.feature.delta(mfcc)
    mfcc_deltadelta = librosa.feature.delta(mfcc, order=2)
    mfcc_m = mfcc.mean(axis=1)
    mfcc_v = mfcc.var(axis=1)
    mfcc_delta_m = mfcc_delta.mean(axis=1)
    mfcc_deltadelta_m = mfcc_deltadelta.mean(axis=1)
    mfcc_delta_v = mfcc_delta.var(axis=1)
    mfcc_deltadelta_v = mfcc_deltadelta.var(axis=1)
    
    for mf_m in mfcc_m:
        features.append(mf_m)
    for mf_v in mfcc_v:
        features.append(mf_v)
    for mfd_m in mfcc_delta_m:
        features.append(mfd_m)
    for mfd_v in mfcc_delta_v:
        features.append(mfd_v)
    for mfdd_m in mfcc_deltadelta_m:
        features.append(mfdd_m)
    for mfdd_v in mfcc_deltadelta_v:
        features.append(mfdd_v)
    #freq_div = [4,8,16,32,64,128]
    #time_div = [7,13,14,26,91]
    freq_div = [8]
    time_div = [7]
...
def col_names_generator(signal_n_samp, sample_rate):
    features_name = []
    for k in range(13):
        features_name.append(str(k)+' mfcc_m')
    for k in range(13):
        features_name.append(str(k)+' mfcc_v')
    for k in range(13):
        features_name.append(str(k)+' mfcc_delta_m')
    for k in range(13):
        features_name.append(str(k)+' mfcc_delta_v')
    for k in range(13):
        features_name.append(str(k)+' mfcc_deltadelta_m')
    for k in range(13):
        features_name.append(str(k)+' mfcc_deltadelta_v')
    for i in range(7*8):
        features_name.append(str(i)+' mean_spec')
        features_name.append(str(i)+' var_spec')    
...
if __name__ == '__main__':
    ...
    final.to_csv('evaluation_set_mfcc_delta_MelSpec.csv')
