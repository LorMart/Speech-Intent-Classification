...
def create_dataframe(df, start=10, end=20):
    ...
    for i in tqdm(range(start, end)):
        ...
        RMS.append(extract_RMS(audio_trimmed, sample_rate))
    
    features_name = col_names_generator(len(RMS[0]))
    new_df = pd.DataFrame(RMS, columns = features_name)
