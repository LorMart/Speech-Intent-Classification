# -*- coding: utf-8 -*-
"""
Created on Wed Jan 25 17:36:24 2023
...
path_Dev = "dsl_data/evaluation.csv"
...
def col_names_generator(size):
    features_name = []
    for i in range(size):
        features_name.append('ZCR '+str(i)+':')
...
def create_dataframe(df, start=10, end=20):
    ...
    for i in tqdm(range(start, end)):
        ...
        zcr_list.append(zero_crossing_rate(audio_trimmed, center = False,frame_length=4096, hop_length=2048)[0])
    
    features_name = col_names_generator(len(zcr_list[0]))

    new_df = pd.DataFrame(zcr_list, columns = features_name)
    return new_df

    
def main():
    print('ciao')
    #values_L = ((, 0, 1232), (, 1232, 2*1232),
    #           (, 2*1232, 3*1232), (, 3*1232,  4*1232))
    #values = ((df, 0, 2360), (df, 2360, 2*2360), (df, 2360*2, 3*2360),(df, 3*2360,  9440))
    values = ((df,0, 363), (df,363, 2*363), (df,2*363, 3*363), (df, 3*363, 1455))
    #values_test = ((,0, 10), (,10, 20), (,20, 30), (, 30, 40))
...
if __name__ == '__main__':
    ...
    final.to_csv('ZCR_on_evaluation_set_4096_2048.csv')
