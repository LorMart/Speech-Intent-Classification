...
def extract_centroid(signal, sample_rate):
    ...
    return librosa.feature.spectral_centroid(S=S, window='blackmanharris')[0]

def col_names_generator(vett_len):
    features_name = []
    
    for i in range(vett_len):
        features_name.append('Centroid '+str(i)+':')
...
def create_dataframe(df, start=10, end=20):
    ...
    for i in tqdm(range(start, end)):
        ...
        centroids.append(extract_centroid(audio_trimmed, sample_rate))
    
    features_name = col_names_generator(len(centroids[0]))
    new_df = pd.DataFrame(centroids, columns = features_name)
...
def main():
    ...
    values = ((df, 0, 2360), (df, 2360, 2*2360), (df, 2360*2, 3*2360),(df, 3*2360,  9440))
...
if __name__ == '__main__':
    ...
    final.to_csv('dev_set_Spectral_centroid.csv')
